...
import { Avatar, Badge, TerrainBadge } from '@/components/ui';
...
interface FilmerProfileProps {
  params: Promise<{ id: string }>;
}

function FilmerProfileInner({ params }: FilmerProfileProps) {
  const { id } = use(params);
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        console.log('[FilmerProfile] Fetching filmer with id:', id);
        const [filmerData, sessionsData, reviewsData] = await Promise.all([
          getUser(id),
          getFilmerSessions(id),
          getFilmerReviews(id),
        ]);
        console.log('[FilmerProfile] Fetched filmerData:', filmerData);
...
export default function FilmerProfile({ params }: FilmerProfileProps) {
  return (
    <Suspense fallback={<FilmerProfileLoading />}>
      <FilmerProfileInner params={params} />
    </Suspense>
  );
}
